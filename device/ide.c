...
// 		// 如果进入 ide_read 时磁盘还在 DRQ 状态，说明上一个ide的读取操作还没有完成
// 		if (inb(reg_status(hd->my_channel)) & 0x08) {
// 			PANIC("IDE Error: DRQ is set before command out. Previous read was incomplete!");
// 		}
...
void ide_read(struct disk* hd, uint32_t lba, void* buf, uint32_t sec_cnt) {
    ...
    // 无论读多少(1, 7, 13, 256...)，一次性告知硬盘
    select_sector(hd, lba, sec_cnt); 
    cmd_out(hd->my_channel, CMD_READ_SECTOR);

    uint32_t secs_done = 0;
   	while(secs_done < sec_cnt) {
        hd->my_channel->expecting_intr = true; // 每次等待前重新开启
        sema_wait(&hd->my_channel->wait_disk); 
...
void ide_write(struct disk* hd, uint32_t lba, void* buf, uint32_t sec_cnt) {
    ...
    uint32_t secs_done = 0;
    while(secs_done < sec_cnt) {
        // 检查 BSY 和 DRQ。写操作前必须确认硬盘准备好接收数据
        if(!busy_wait(hd)){
            PANIC("ide_write: busy_wait timeout!");
        }

        // 每次只写 1 个扇区 (512字节)
        write2sector(hd, (void*)((uint32_t)buf + secs_done * SECTOR_SIZE), 1);
