...
// get [pg_cnt] continuous virtual pages
static void* get_vpages(uint32_t* pgdir_vaddr,uint32_t pg_cnt,bool user){
	uint32_t pte_idx=0,pde_idx=0,pgdir_item_end=0;
	uint32_t pde_start = 0,pte_start = 0;

	if(user){
		// USER_VADDR_START is 0x8048000
		pde_start = 0x20;
		pte_start = 0x48;
		pgdir_item_end = USER_PDE_NR;
	}else{
		// K_HEAP_START is 0xc0100000
		pde_start=768;
		pte_start = 256;
		pgdir_item_end = TABLE_ITEM_NR;
	}

	pte_idx = pte_start;
	pde_idx = pde_start;

	if(pg_cnt<=TABLE_ITEM_NR){
		// first, we check the valid pde
		while(pde_idx<pgdir_item_end){
			uint32_t* v_pde_ptr = pgdir_vaddr+pde_idx;
			uint32_t pde_item = *v_pde_ptr;
			if(!(pde_item&0xfffff000)){ // it is free pde
				pde_idx++;
				continue;
			}
			uint32_t* page_table_addr = (uint32_t*)PAGE_TABLE_VADDR(pde_idx);
			// find free continuous pte
			while(pte_idx<TABLE_ITEM_NR){
				uint32_t* v_pte_ptr = page_table_addr+pte_idx;
				uint32_t pte_item = *v_pte_ptr;
				if((pte_item&0xfffff000)){ // it is used pte
					pte_idx++;
					continue;
				}else{
					uint32_t continuous_free_pte_nr = 0;
					while(pte_idx+continuous_free_pte_nr<TABLE_ITEM_NR&&!(*(v_pte_ptr+continuous_free_pte_nr)&0xfffff000)){ // slide window
						continuous_free_pte_nr++;
					}
					if(continuous_free_pte_nr>=pg_cnt){
						return (void*)PTE_TO_VADDR(pde_idx,pte_idx);
					}else{
						pte_idx += (continuous_free_pte_nr+1);
						continue;
					}
				} 
			}
		...
		// second, we check the invalid(free) pde
		while(pde_idx<pgdir_item_end){
			uint32_t* v_pde_ptr = pgdir_vaddr+pde_idx;
			uint32_t pde_item = *v_pde_ptr;
			if(pde_item&0xfffff000){ // it is used pde
				pde_idx++;
				continue;
			}
			// it is free pde
			uint32_t* page_table_phy_addr = (uint32_t*)palloc(&kernel_pool);
			if(page_table_phy_addr==NULL){
				PANIC("get_vpages:::page_table_phy_addr palloc failed!\n");
			}
			*v_pde_ptr = ((uint32_t)page_table_phy_addr<<12)|PG_US_U|PG_RW_W|PG_P_1;
			return (void*)PTE_TO_VADDR(pde_idx, pte_idx);
		}
		PANIC("get_vpages:::page_table_phy_addr palloc failed!\n");
	}else{
		uint32_t remain_v_pg = pg_cnt%TABLE_ITEM_NR;
		uint32_t need_pde_nr = DIV_ROUND_UP(pg_cnt,TABLE_ITEM_NR);
		while(pde_idx<pgdir_item_end){
			uint32_t* v_pde_ptr = pgdir_vaddr+pde_idx;
			uint32_t pde_item = *v_pde_ptr;
			// we check used pde first
			if(!(pde_item&0xfffff000)){ // it is free pde
				pde_idx++;
				continue;
			}

			uint32_t loose_need_pde_nr = remain_v_pg==0?need_pde_nr:need_pde_nr-1;
			uint32_t continuous_free_pde_nr = 0;
			
			// loose check
			while((*(v_pde_ptr+continuous_free_pde_nr+1)&0xfffff000)==0){
				continuous_free_pde_nr++;
			}
			if(continuous_free_pde_nr<loose_need_pde_nr){
				pde_idx += (continuous_free_pde_nr+1);
				continue;
			}


			uint32_t* page_table_addr = (uint32_t*)PAGE_TABLE_VADDR(pde_idx);
			int32_t backward_pte_idx = TABLE_ITEM_NR-1;
			uint32_t last_pte_nr = 0;
			while(backward_pte_idx>=0){
				uint32_t* v_pte_ptr = page_table_addr+backward_pte_idx;
				uint32_t pte_item = *v_pte_ptr;
				
				// check if the page dose not exist in the mem
				if(!(pte_item&0xfffff000)){ // if pte not in use
					last_pte_nr++;
					backward_pte_idx--;
					continue;
				}else{
					// 2050-> %=2 /up=3  [1023] 1024 3; [2] 1024 1024; [1] 1024 1024 1 
					// 2048-> %=0 /up=2  [1023] 1024 1; [0] 1024 1024
					if(last_pte_nr>=remain_v_pg&&remain_v_pg!=0){
						// no need to round up
						need_pde_nr--;
					}
					break;
			...
			if(continuous_free_pde_nr>=need_pde_nr){
				//return result
				// backward_pte_idx points to a used pte
				uint32_t alloc_idx = 1;
				
				while(alloc_idx<=need_pde_nr){
					// v_pde_ptr points to a used pde
					// so alloc_idx start from 1
					void* page_table_phy_addr = palloc(&kernel_pool);
					*(v_pde_ptr+alloc_idx)= ((uint32_t)page_table_phy_addr<<12)|PG_US_U|PG_RW_W|PG_P_1;
					alloc_idx++;

				}
				// return start vaddr
				uint32_t start_pte_idx = backward_pte_idx+1;
				return (uint32_t*)PTE_TO_VADDR(pde_idx, start_pte_idx);
		...
		// next, we check the free pde
		pde_idx = pde_start;
		pte_idx = 0;
		while(pde_idx<pgdir_item_end){
			uint32_t* v_pde_ptr = pgdir_vaddr+pde_idx;
			uint32_t pde_item = *v_pde_ptr;
			if((pde_item&0xfffff000)!=0){ // if it is a used pde
				pde_idx++;
				continue;
			}
			uint32_t continuous_free_pde_nr = 0;
			// slide window
			while((pde_idx+continuous_free_pde_nr)<pgdir_item_end&&
				  (*(v_pde_ptr+continuous_free_pde_nr)&0xfffff000)==0){
				continuous_free_pde_nr++;
			}
			...
			// v_pde_ptr points to a unused pde, so allocate start from 0
			...
			// return result
			// compute the start vaddr which will be mapped in the v_pde_ptr
			return (void*)PTE_TO_VADDR(pde_idx,pte_idx);
		}
		PANIC("get_vpages::alloc vaddr failed! dont has enough virtual pages!\n");
	}
	PANIC("get_vpages:::should not be here!\n");
}
