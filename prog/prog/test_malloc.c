...
int main(void) {
    ...
    check(brk2 > brk1, "large allocation should further grow heap");

    free(big);
    uint32_t brk3 = (uint32_t)sbrk(0);
    printf("test_malloc: brk after freeing tail large block = %x\n", brk3);
    check(brk3 == brk1, "freeing tail large block should trim heap back");
    ...
    check(brk6 < brk5, "freeing tail block should trim heap");

    free(a);
    uint32_t brk7 = (uint32_t)sbrk(0);
    printf("test_malloc: brk after freeing a = %x\n", brk7);
    check(brk7 <= brk1, "freeing remaining top blocks should continue trimming");
    ...
    check(brk8 == brk0, "all tail allocations freed should return brk to initial value");
