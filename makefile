...
hd:
	dd if=$(BUILD_DIR)/mbr.bin of=$(BOCHS_DIR)/hd60M.img count=1 bs=512 conv=notrunc
	dd if=$(BUILD_DIR)/loader.bin of=$(BOCHS_DIR)/hd60M.img count=4 bs=512 conv=notrunc seek=2
	dd if=$(BUILD_DIR)/kernel.bin of=$(BOCHS_DIR)/hd60M.img bs=512 count=500 seek=9 conv=notrunc 
