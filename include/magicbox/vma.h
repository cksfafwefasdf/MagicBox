...
struct vm_area {
    ...
    struct inode* vma_inode; // 映射的文件（如果是匿名内存如堆栈，则为 NULL）
...
extern void add_vma(struct task_struct* task, uint32_t start, uint32_t end, uint32_t pgoff, struct inode* inode, uint32_t flags, uint32_t filesz); 
