...
int32_t pcb_fd_install(int32_t global_fd_idx){
	...
	while(local_fd_idx<MAX_FILES_OPEN_PER_PROC){
		if(cur->file_table->fd_table[local_fd_idx].global_fd_idx==-1){
			cur->file_table->fd_table[local_fd_idx].global_fd_idx = global_fd_idx;
			cur->file_table->fd_table[local_fd_idx].flags = 0;
			break;
		}
		local_fd_idx++;
	}
...
uint32_t fd_local2global(struct task_struct* task , uint32_t local_fd){
	int32_t global_fd = task->file_table->fd_table[local_fd].global_fd_idx;
